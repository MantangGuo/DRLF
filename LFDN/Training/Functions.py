...
def CropLF(lf,patchSize, stride): #lf [u,v,x,y,c]
...
def MergeLF(lfStack, coordinate, overlap):
    ...
    xMerged=coordinate[0]*x-(coordinate[0]+1)*overlap
    yMerged=coordinate[1]*y-(coordinate[0]+1)*overlap
    ...
    for i in range(coordinate[0]):
        for j in range(coordinate[1]):
            lfMerged[:,
                     :,
                     :,
                     i*(x-2*overlap):(i+1)*(x-2*overlap),
                     j*(y-2*overlap):(j+1)*(y-2*overlap),
                     :]=lfStack[:,
                                i*(coordinate[0]-1)+j,
                                :,
                                :,
